...
export default class ContactForm extends React.Component {
    ...
    onScanResults (data) {
        if (data.startsWith('zip:')) {
            const tag = 'zip:'
            const accountNumber = data.substring(tag.length, data.length)
            this.setState({accountNumber})
        } else {
            console.error("Unrecognised format: " + data)
        }
    ...
    render () {
        return <div className="form" style={{padding: '0.2em'}}>
            ...
            <FormCard title="Enter Details">
                <TextField style={{margin: '0 1em'}}
                  floatingLabelText="First name"
                  value={this.state.firstName}
                  onChange={this.onTextFieldChange('firstName').bind(this)} />
                <br />

                <TextField style={{margin: '0 1em'}}
                  floatingLabelText="Last name"
                  value={this.state.lastName}
                  onChange={this.onTextFieldChange('lastName').bind(this)} />
                <br />
                ...
                <div>
                    <TextField style={{margin: '0 1em'}}
                      ...
                      onChange={this.onTextFieldChange('accountNumber').bind(this)} />

                    <IconButton
                      onTouchTap={this.doScanCode.bind(this)}>
                        <QRCodeIcon />
                    </IconButton>
                </div>
            </FormCard>
            ...
            <FloatingActionButton
              style={{float: 'right', margin: '-1.5em 1.5em 0 0', color: '#00ff00'}}
              onTouchTap={_ => {this.props.onSubmit(this.state)}}>
                <ActionDone />
            </FloatingActionButton>
        </div>
    }
