...
export default class TransactionList extends React.Component {
    constructor(props) {
        ...
        this.doShowTransaction = this.doShowTransaction.bind(this)
    }

    doShowTransaction(txid) {
        let win = window.open('https://etherscan.io/tx/' + txid)
        win.focus()
    }

    render() {
        return <List>
            {this.props.model.map(item => <ListItem primaryText={item}
                onTouchTap={() => {this.doShowTransaction(item)}} />)}
        </List>
    }
}
