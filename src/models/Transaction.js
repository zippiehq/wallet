...
export default class Transaction {
    static new(params) {
        const transactions = JSON.parse(localStorage.getItem('accounts') || '[]')
    ...
    static findById(id) {
        let transactions = JSON.parse(localStorage.getItem('accounts') || '[]')

        for(var i = 0; i < transactions.length; i++) {
            if(transactions[i].txid === id) return transactions[i]
        }
