...
/* ContactCreatePage
 *  - Renders a contact form, for creating new contacts.
 */
export default class ContactCreatePage extends React.Component {
    doCreate (formdata) {
        if (formdata.firstName.length === 0 ||
            formdata.lastName.length === 0 ||
            formdata.accountNumber.length !== 42) {
            console.error("You must supply all form parameters.")
            return
        }
    ...
    render () {
        return <div className="page">
            <ContactForm
              firstName={this.props.location.query.firstName}
              lastName={this.props.location.query.lastName}
