...
/* ContactUpdatePage
 *  - Renders a contact list embedded into an application page.
 */
export default class ContactListPage extends React.Component {
    ...
    componentDidMount() {
        this.store.events.bind('change', this.forceUpdate.bind(this))
    }

    componentWillUnmount() {
        this.store.events.unbind('change', this.forceUpdate.bind(this))
    }

    onCreateRequest(ev) {
        ev.stopPropagation()
        this.props.router.push('/contacts/create')
    }
    
    onItemSelected(ev, contact) {
        ev.stopPropagation()

        const query = this.props.location.query

        // Are we a contact selector?
        //FIXME: Does router have history through hashHistory we can use to get referrer?
        if(query.mode === 'select') {
            this.props.router.replace(query.dest + "&to=" + contact.accountNumber)
            return
        }

        this.props.router.push('/contacts/' + contact.accountNumber)
    }
    
    render() {
        return <div className="page">
            <ContactList
              model={this.store.contacts}
              onCreateRequest={this.props.location.query.mode !== 'select' ? this.onCreateRequest : null}
